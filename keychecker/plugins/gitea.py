"""
Gitea-specific provider plugin for KeyChecker.
"""
...
class GiteaProvider(BaseGitProvider):
    ...
    def __init__(
        ...
        config = ServerConfig(
            name="gitea", hostname="gitea.com", port=22, username="git"
        )
        super().__init__(config, timeout, concurrency, show_progress)

    async def validate_key(self, private_key_path: str) -> Dict[str, Any]:
        """
        Validate SSH key against Gitea and extract user information.

        Gitea returns specific banner format with username.
        """
        ...
        # Check for Gitea success patterns
    ...
    async def identify_user(self, private_key_path: str) -> Optional[str]:
        """
        Identify the Gitea username associated with the private key.
        """
    ...
    async def discover_organizations(
        ...
        """
        Discover Gitea organizations.

        TODO: Implement Gitea organization discovery.
        """
    ...
    async def test_repository_access(
        ...
        """
        Test Gitea repository access.
        """
        repo_url = f"git@gitea.com:{owner}/{repo_name}.git"
