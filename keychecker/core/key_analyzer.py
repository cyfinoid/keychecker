...
class SSHKeyAnalyzer:
    ...
    def analyze_key_file(self, key_path: str) -> Dict[str, Any]:
        ...
        if private_key is None:
            ...
            if any(key_data_str.strip().startswith(prefix) for prefix in ['ssh-rsa', 'ssh-ed25519', 'ecdsa-sha2-', 'ssh-dss']):
                raise ValueError("This appears to be a public key file, not a private key. Please provide the private key file (usually without .pub extension)")
            
            if 'BEGIN OPENSSH PRIVATE KEY' in key_data_str:
                raise ValueError("OpenSSH private key detected but couldn't parse - may be passphrase protected or corrupted")
            elif 'BEGIN' in key_data_str and 'PRIVATE KEY' in key_data_str:
                raise ValueError("PEM private key detected but couldn't parse - may be passphrase protected or corrupted")
            elif 'BEGIN' in key_data_str and 'PUBLIC KEY' in key_data_str:
                raise ValueError("This appears to be a public key file, not a private key. Please provide the private key file")
            else:
                raise ValueError("File doesn't appear to contain a recognized private key format")
    ...
    def _get_key_info(self, private_key, public_key) -> Dict[str, Any]:
    ...
    def _generate_public_key_string(self, public_key, key_type: str) -> str:
        ...
        return ssh_public_key.decode('utf-8').strip()
    ...
    def _generate_fingerprint(self, public_key_str: str, hash_type: str = 'sha256') -> str:
    ...
    def _analyze_security(self, key_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if key_type in self.DEPRECATED_ALGORITHMS or algorithm in self.DEPRECATED_ALGORITHMS:
        ...
        if key_type == 'rsa' and bits in self.INSECURE_RSA_SIZES:
            ...
            flags['warnings'].append(f"RSA key size {bits} is below recommended 2048 bits")
    ...
    def _extract_insights(self, comment: Optional[str]) -> Dict[str, Any]:
        ...
        ip_matches = re.findall(ip_pattern, comment)
        if ip_matches:
